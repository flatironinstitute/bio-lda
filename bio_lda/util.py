...
def generate_dataset(samples=1000, d=50, proportions=[0.5,0.5]):
    mat = np.random.randn(d,d)
    cov = mat@mat.T

    mean1 = np.random.randn(d)
    mean2 = np.random.randn(d)
...
def scikit_LDA(X,y):
    ...
    return y_, true_lda_score


def run_offline(X, y, mean1, mean2, cov, true_lda_score, epochs=5000):
